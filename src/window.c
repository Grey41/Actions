...
static int window_set_title(PyObject *self, PyObject *value, void *closure) {
    ...
    return window.title = strdup(title), 0;
}
...
static int window_set_red(PyObject *self, PyObject *value, void *closure) {
    DEL(value, "red")
    return ERR(window.color.x = PyFloat_AsDouble(value)) ? -1 : window_clear();
}
...
static int window_set_green(PyObject *self, PyObject *value, void *closure) {
    DEL(value, "green")
    return ERR(window.color.y = PyFloat_AsDouble(value)) ? -1 : window_clear();
}
...
static int window_set_blue(PyObject *self, PyObject *value, void *closure) {
    DEL(value, "blue")
    return ERR(window.color.z = PyFloat_AsDouble(value)) ? -1 : window_clear();
}

static Vector *window_get_color(PyObject *self, void *closure) {
    Vector *vect = vector_new(NULL, (double *) &window.color, 3, (int (*)(PyObject *)) window_clear);
...
static int window_set_color(PyObject *self, PyObject *value, void *closure) {
    DEL(value, "color")
    return vector_set(value, (double *) &window.color, 3) ? -1 : window_clear();
}
...
static int window_set_width(PyObject *self, PyObject *value, void *closure) {
    DEL(value, "width")
    return ERR(window.size.x = PyFloat_AsDouble(value)) ? -1 : window_size();
}
...
static int window_set_height(PyObject *self, PyObject *value, void *closure) {
    DEL(value, "height")
    return ERR(window.size.y = PyFloat_AsDouble(value)) ? -1 : window_size();
}

static Vector *window_get_size(PyObject *self, void *closure) {
    Vector *vect = vector_new(NULL, (double *) &window.size, 2, (int (*)(PyObject *)) window_size);
...
static int window_set_size(PyObject *self, PyObject *value, void *closure) {
    DEL(value, "size")
    return vector_set(value, (double *) &window.size, 2) ? -1 : window_size();
}
...
static int window_init(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    INIT(!PyArg_ParseTupleAndKeywords(
        args, kwds, "|sddO:Window", kwlist, &title, &window.size.x,
        &window.size.y, &color) || vector_set(color, (double *) &window.color, 4))

    window.title = strdup(title);
    SDL_SetWindowTitle(window.sdl, title);

    return window_size(), window_clear();
}
